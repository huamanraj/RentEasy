...
const FlatForm = ({ onSubmit, isSubmitting = false, initialData = null, isEditMode = false, submitButtonText = 'Post Flat' }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    video: null, // Will store File object
    isAvailable: true,
    // Add separate latitude and longitude fields instead of nested location object
    latitude: null,
    longitude: null,
    location: null, // Keep this for internal use, won't be sent to API
    ownerNumber: '' // Add owner's contact number
  });
  ...
  useEffect(() => {
    if (initialData) {
      setFormData(prev => ({
        ...
        // Don't set images and video here as they need special handling
      }));
  ...
  const handleImageChange = (e) => {
    ...
    // Validate file count - enforce maximum 5 images total
  ...
  useEffect(() => {
    if (location && location.latitude && location.longitude) {
      setFormData(prevData => ({
        ...prevData,
        // Store as separate fields and in the location object for internal use
  ...
  useEffect(() => {
    if (success) {
      ...
      setFormData({
        ...
        longitude: null,
        location: null,
        ownerNumber: '' // Reset owner number
      });
  ...
  // Add a toggle handler for availability
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 p-6 bg-white rounded-lg shadow border border-gray-200">
      ...
      {/* Add owner's contact number field - just before the submit button */}
      <div>
